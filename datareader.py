...
class YcbineoatReader:
  def __init__(self,video_dir, downscale=1, shorter_side=None, zfar=np.inf, K=None):
    self.video_dir = video_dir
    self.downscale = downscale
    self.zfar = zfar
    self.color_files = sorted(glob.glob(f"{self.video_dir}/rgb/*.png"), key=lambda x: int(x.split('_')[-1].split('.')[0]))
    if K is None:
    ...
    self.H,self.W = 480, 640 #cv2.imread(self.color_files[0]).shape[:2]
  ...
  def get_mask(self,i, directory):
    mask = cv2.imread(f"{directory}/masks_{i}.png")
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
  ...
  def get_depth(self,i):
    depth = cv2.imread(self.color_files[i].replace('rgb','depth'),-1)
    if depth is None: 
      depth = cv2.imread(self.color_files[i].replace('img','depth').replace('rgb', 'depth'),-1) 
    depth = depth/1e3
